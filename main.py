...
def main():
    ...
    for iteration_version in tqdm(range(args.iteration_amount)):
        ...
        answer_dataset = answer_inference(model, tokenizer, answer_prompt_dataset,debug)

        time2 = time.time()
        
        # Use clustering algorithm to get new examples.
        new_example_indices_dict = get_new_examples(bert,bert_tokenizer,cur_iter_root_path, answer_dataset, args.clusters, debug)
